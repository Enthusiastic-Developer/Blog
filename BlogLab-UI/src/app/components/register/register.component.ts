...
export class RegisterComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.registerForm = this.formBuilder.group(
      ...
      {
        validators: this.matchValue('password', 'confirmPassword'),
      }
    );
  }
  ...
  matchValue(password: string, confirmPassword: string): ValidatorFn {
    return (formGroup: AbstractControl): { [key: string]: any } | null => {
      ...
      if (
        ...
        !confirmPasswordControl.errors['isMatching']
      ) {
        return null;
      }

      if (passwordControl.value !== confirmPasswordControl.value) {
        confirmPasswordControl.setErrors({ isMatching: true });
